...
> 上线文档更新版（功能与 1.4.0 一致）。

### 变更
- 新建本文件（CHANGELOG.md）：v1.0.0→v1.4.0 全版本上线记录 + 实战交付页清单。
- README.md：徽章对齐（Modules-19 / UI Styles-13 / Role Presets-11）、特性补 4 新模块与 claw、新增「版本与上线记录」段落。
- SKILL.md：装配流程段落遗留旧计数修正（15 模块/5 风格/10 配方 → 19/13/11）。

### 发布记录
- Git：`f2a2016`（文档）+ 本版 commit（版本号 1.4.1）。
- SkillHub：v1.4.1 发布成功（27 文件白名单 zip，skillId 156574）。
